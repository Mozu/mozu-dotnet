...
namespace Mozu.Api.ToolKit.Events
{
    ...
    {
        private readonly ILogger _logger = LogManager.GetLogger(typeof (Api.Events.EventService));
        ...
        public void ProcessEvent(IApiContext apiContext, Event eventPayLoad)
        {
            ...
            if (topic != "application" && !IsAppEnabled(apiContext) )
            {
            ...
            eventProcessor.Process(_container,apiContext, eventPayLoad);

        }

        private bool IsAppEnabled(IApiContext apiContext)
        {
            var applicationResource = new ApplicationResource(apiContext);
            var application = applicationResource.ThirdPartyGetApplication();
