...
My local runline
codegen.ps1 -Major 1 -Minor 9 -Env "ci" -GenPath "\\\\dev.volusion.com\\data\\QA\\integrations\\ImportExport" -CodeGenBaseUrl "http://apicodegen.mozu-ci.com/" -AppClaim "" -RunPublisher:$false 
#>
...
try {
    $response = Invoke-WebRequest -Method Get -Uri "$CodeGenServer/sdkcodegen/$env/versions"  -ContentType "application/json" # -Proxy "http://localhost:8888"
...
try 
{
    $response = Invoke-WebRequest -Method Post -Uri "$CodeGenServer/sdkcodegen" -Body $body -Headers $headers -ContentType "application/json" -ErrorAction Stop
    Write-Host $response.StatusCode
    Write-Host $response.Headers
}
catch [System.Net.WebException] 
